...
output <- foreach(l=1:N, .combine=cbind, .export = c("simWind", "angleMatrix","xScalingFactor", "rDirLine", "KofA", "Lest", "sqrt.n",
                                                     "testAniso","cylK", "GLoc", "periodogram", "thetaSpectrum", "rvonmises",
                                                     "owin", "xdim", "ydim", "theta",
                                                     "grid_length", "M", "Gepsilon", "Kaspect", "Tepsilon", "Tp", "theta.grid",
                                                     "as.ppp", "r.grid.W","boot.test_GKF", "loaded", "ixs"),
                  .multicombine = TRUE, .maxcombine = N) %dopar% {
                    ...
                    pat[[1]] <- pat[[1]][abs(pat[[1]][,1])<0.25 & abs(pat[[1]][,2])<0.25,]
...
write.csv(output, "GIBBS_08_TILING_4_SMALL.csv")
