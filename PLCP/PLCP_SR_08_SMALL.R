...
output <- foreach(l=1:N, .combine=cbind, .export = c("simWind", "angleMatrix","xScalingFactor", "rDirLine", "KofA", "Lest", "Hest",
                                                     ...
                                                     "as.ppp", "r.grid.W", "rHomPois", "iterSR"),
                  .multicombine = TRUE, .maxcombine = N) %dopar% {
                    ...
                    for(m in 1:M){
                      if(l==888){
                        recPat <- stchRecH(spatPat = pat, iter=iterSR, rmax=0.582301740, rlength=ceiling(0.582301740)/0.000977)
                      } else {
                        recPat <- stchRecH(spatPat = pat, iter=iterSR)
                      }
...
write.csv(output, "PLCP_SR_08_SMALL.csv")