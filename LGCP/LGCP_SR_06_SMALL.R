...
output <- foreach(l=1:N, .combine=cbind, .export = c("simWind", "angleMatrix","xScalingFactor", "rLGCPtransformed", "rAnisotrop", "Lest", "Hest",
                                                     "testAniso","cylK", "GLoc", "periodogram", "thetaSpectrum", "stchRecH",
                                                     "owin", "xdim", "ydim", "var", "scale", "mu", "model", "rLGCP",
                  ...
                  .multicombine = TRUE, .maxcombine = N) %dopar% {
                    ...
                    for(m in 1:M){
                      recPat <- stchRecH(pat, iter=iterSR)
...
write.csv(output, "LGCP_SR_06_SMALL.csv")