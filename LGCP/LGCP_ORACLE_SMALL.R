...
vGtile.W <- vKtile.W <- vTtile <- matrix(NA, nrow=grid_length, ncol=M)
for(i in 1:M){
  set.seed(seeds[i])
  ...
  vTtile[,i] <- thetaSpectrum(Theta, theta.grid, Tepsilon)
  print(i)
}
...
output <- foreach(l=1:N, .combine=cbind, .export = c("simWind", "angleMatrix", "xScalingFactor", "rAnisotrop", "rLGCPtransfprmed", "Lest",
                                                     "testAniso","cylK", "GLoc", "periodogram", "thetaSpectrum", "rLGCP",
                                                     "owin", "xdim", "ydim", "mu", "var", "scale", "model",
                                                     "theta", "grid_length", "M", "Gepsilon", "Kaspect", "Tepsilon", "Tp", "theta.grid",
                                                     "as.ppp", "r.grid.W", "patterns", "vGtile.W", "vKtile.W", "vTtile"),
                  .multicombine = TRUE, .maxcombine = N) %dopar% {
                    ...
                    Gpref2 <- GLoc(spatPat=pat, alpha=theta, epsilon=Gepsilon, r=r.grid.W, contribution=FALSE, distance=Gpref$distance, angles=Gpref$angles)
                    Gsec <- GLoc(spatPat=pat, alpha=secDir, epsilon=Gepsilon, r=r.grid.W, contribution=FALSE, distance=Gpref$distance, angles=Gpref$angles)
                    Gvec.L <- Gpref2-Gsec
                    ...
                    vGtile.L  <- vKtile.L <- matrix(NA, nrow= length(r.grid.W), ncol= M)
                    ...
                    for(m in 1:M){
                      ...
                      prefG <- GLoc(spatPat=recPat, alpha=theta, epsilon=Gepsilon, r=r.grid.L, retMatrices = TRUE)
                      secG <- GLoc(spatPat=recPat, alpha=secDir, epsilon=Gepsilon, r=r.grid.L,
                                   distance=prefG$distance, angles=prefG$angles)
                      vGtile.L[,m] <- prefG$Gloc - secG
...
write.csv(output, "LGCP_ORACLE_1_SMALL.csv")
...
write.csv(output, "LGCP_ORACLE_08_SMALL.csv")
...
write.csv(output, "LGCP_ORACLE_06_SMALL.csv")
...
write.csv(output, "LGCP_ORACLE_04_SMALL.csv")
