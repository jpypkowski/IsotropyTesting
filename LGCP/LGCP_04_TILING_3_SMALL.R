...
# Only contains G, K, and theta spectrum

boot.test_GKF <- function(spatPat, sqrt.n, reps, r.grid.W, r.grid.L, theta.grid, 
...
r.grid.W <- seq(0, 0.25/2, length=37)[-1]
...
output <- foreach(l=1:N, .combine=cbind, .export = c("simWind", "angleMatrix","xScalingFactor", "rAnisotrop", "rLGCPtransformed", "Lest", "sqrt.n",
                                                     "testAniso","cylK", "GLoc", "periodogram", "thetaSpectrum", "stchRecH", "rLGCP",
                                                     "owin", "xdim", "ydim", "var", "scale", "mu", "model",
                                                     "theta", "grid_length", "M", "Gepsilon", "Kaspect", "Tepsilon", "Tp", "theta.grid",
                                                     "as.ppp", "r.grid.W","boot.test_GKF"),
                  .multicombine = TRUE, .maxcombine = N) %dopar% {
                    ...
                    Gpref <- GLoc(spatPat=pat, alpha=theta, epsilon=Gepsilon, r=r.grid.L, contribution=FALSE, retMatrices=TRUE)
                    Gsec <- GLoc(spatPat=pat, alpha=secDir, epsilon=Gepsilon, r=r.grid.L, contribution=FALSE)
                    Gvec.L <- Gpref$Gloc-Gsec
                    ...
                    vTiles <- boot.test_GKF(spatPat=pat, sqrt.n=sqrt.n, reps = M, r.grid.W=r.grid.W, r.grid.L=r.grid.L, theta.grid=theta.grid, 
                                              prefDir=theta, Gepsilon=Gepsilon, Kaspect=Kaspect, Tepsilon=Tepsilon, Tp=Tp)
                    
                    
                    test.GW <- testAniso(Gvec.W, vTiles$vGtile.W, "G")
                    test.GL <- testAniso(Gvec.L, vTiles$vGtile.L, "G")
                    test.KW <- testAniso(Kvec.W, vTiles$vKtile.W, "K")
                    test.KL <- testAniso(Kvec.L, vTiles$vKtile.L, "K")
                    test.T <- testAniso(Tvec, vTiles$vTtile, "T")
                    print(l)
...
write.csv(output, "LGCP_04_TILING_3_SMALL.csv")